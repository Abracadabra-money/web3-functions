...
import { createClient, fetchExchange, gql, TypedDocumentNode } from '@urql/core';
...
const userLocksQuery: TypedDocumentNode<UserLocksResponse, { lockLimit: number; minimumUnlockTime: number }> = gql`
...
Web3Function.onRun(async ({ userArgs }: Web3FunctionContext) => {
  const { execAddress, subgraphUrl, lockLimitPerExecution } = userArgs as MsrUserArgs;
  ...
  const lockingMultiRewardsContract = new Contract(execAddress, LOCKING_MULTI_REWARDS_ABI);

  return {
    canExec: true,
    callData: [{
      to: execAddress,
      data: lockingMultiRewardsContract.interface.encodeFunctionData("processExpiredLocks", [
        users,
        locks,
      ])
    }]
