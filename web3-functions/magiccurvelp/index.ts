...
Web3Function.onRun(async (context: Web3FunctionContext) => {
	...
	const getSwapInfo = async (
		amount: string,
	): Promise<{ data: string; minOutAmount: string }> => {
		switch (gelatoArgs.chainId) {
			// KAVA, use openocean aggregator
			case 2222: {
				let url = "https://ethapi.openocean.finance/v2/2222/gas-price";
				// biome-ignore lint/suspicious/noExplicitAny: Hack to make multicall3 available on all chains
				...
				console.log(url);
				response = await api.get(url, { timeout: 10_000, retry: 0 }).json();
				return {
					data: response.data,
	...
	const swapRewardToToken = new Contract(
	...
	const totalRewardsBalanceAfterClaiming =
		await harvestor.totalRewardsBalanceAfterClaiming();
	console.log(
		"totalRewardTokenBalance",
		totalRewardsBalanceAfterClaiming.toString(),
	);
	...
	console.log(
		"minSwapRewardToTokenAmount",
		minSwapRewardToTokenAmount.toString(),
	);

	const totalSwapToTokenBalance = (
		await swapRewardToToken.balanceOf(execAddress)
	).add(minSwapRewardToTokenAmount);
	...
	const minLpAmount = await curveLens.calc_token_amount(
		curvePoolAddress,
		curvePoolAddress,
		amountsIn,
		poolNumCoins,
	...
	if (minLpAmount.gt(BigNumber.from(minRequiredLpAmount))) {
		...
		return {
			canExec: false,
			message: "reward balance too low, not minting lp yet",
		};
	}
	...
	const rpcUrl =
		"https://kava-mainnet-archival.gateway.pokt.network/v1/lb/3b9d1dd7";
	console.log(
		`cast send --private-key=$PRIVATE_KEY --rpc-url=${rpcUrl} --legacy ${execAddress} ${callData[0].data}`,
	);
